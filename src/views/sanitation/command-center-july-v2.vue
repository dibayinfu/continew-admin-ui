            ...
            <PanelCard v-if="activeRightTab === 'alarm'" title="" class="right-card">
              ...
              <div class="alarm-table">
                <div class="table-head">
                  <span>时间</span><span>类型</span><span>位置</span><span>已读</span><span>星标</span><span>关联任务</span>
                </div>
                <div v-for="item in filteredAlarmRows" :key="item.id" class="alarm-row clickable" @click="openAlarmDetail(item)">
                  ...
                  <strong :class="item.read ? 'success' : 'warning'">{{ item.read ? '已读' : '未读' }}</strong>
                  <em>{{ item.star ? '★' : '☆' }}</em>
...
const alarmStats = [
  { key: 'today', label: '今日告警', value: 36, tone: 'danger' },
  { key: 'unread', label: '未读', value: 8, tone: 'warning' },
  { key: 'starred', label: '星标', value: 12, tone: 'info' },
]

type AlarmFilter = 'today' | 'unread' | 'starred'
interface AlarmRow { id: string, time: string, name: string, place: string, level: string, star: boolean, read: boolean, taskNo: string, boxNo: string, rule: string, description: string }
...
const alarmRows = ref<AlarmRow[]>([
  { id: 'AL-001', time: '10:31', name: '箱体满溢', place: '马投涧镇牛家窑村', level: 'danger', star: false, read: false, taskNo: 'RW-20260713-001', boxNo: 'XB-MT-012', rule: '垃圾占比 ≥ 90%', description: '小勾臂箱垃圾占比达到 96%，请及时安排车辆清运。' },
  { id: 'AL-002', time: '10:28', name: '箱体高温', place: '龙泉镇白龙庙村', level: 'danger', star: true, read: false, taskNo: '', boxNo: 'DB-LQ-003', rule: '箱体温度 ≥ 65℃', description: '大勾臂箱温度持续升高，当前温度 68℃，请优先处置。' },
  { id: 'AL-003', time: '10:25', name: '箱体低电量', place: '善应镇南善应村', level: 'warning', star: false, read: false, taskNo: '', boxNo: 'XB-SY-021', rule: '设备电量 ≤ 15%', description: '箱体设备电量仅剩 12%，请安排巡检或更换电池。' },
  { id: 'AL-004', time: '10:19', name: '箱体满溢', place: '东风乡徐家口村', level: 'danger', star: true, read: true, taskNo: 'RW-20260713-004', boxNo: 'XB-DF-008', rule: '垃圾占比 ≥ 90%', description: '箱体已满溢，关联收运单正在执行。' },
  { id: 'AL-005', time: '10:12', name: '箱体高温', place: '马家乡李家庄村', level: 'warning', star: false, read: true, taskNo: 'RW-20260713-005', boxNo: 'DB-MJ-005', rule: '箱体温度 ≥ 65℃', description: '箱体温度异常，等待现场核查。' },
  { id: 'AL-006', time: '09:58', name: '箱体满溢', place: '文明大道街道文明村', level: 'warning', star: true, read: true, taskNo: '', boxNo: 'XB-WM-018', rule: '垃圾占比 ≥ 90%', description: '箱体满溢告警已确认，尚未创建收运单。' },
])
const filteredAlarmRows = computed(() => alarmRows.value.filter((row) => {
  if (activeAlarmFilter.value === 'today') return true
  if (activeAlarmFilter.value === 'unread') return !row.read
  return row.star
}))
